  ...
  const initReferenceUI=()=>{
    ...
    if(sidebar){
      if(!sidebar.querySelector('.ref-sidebar-groups')) sidebar.innerHTML=`
        ...
        <a class="ref-partner-card" href="${base}guides/choose-program/"><b>Партнёрская программа для первого теста</b><span>Сначала проверь GEO, правила трафика и условия выплаты, затем переходи к регистрации.</span><em>Выбрать программу <span>→</span></em></a>`;
...
(function(){
  ...
  const iconSvg={
    ...
    profitads:'<svg aria-hidden="true" focusable="false" viewBox="0 0 64 64"><rect width="64" height="64" rx="14" fill="#ff9f1a"></rect><path d="M17 43V31h7v12h-7Zm12 0V23h7v20h-7Zm12 0V15h7v28h-7Z" fill="#fff"></path><path d="M15 49h35" stroke="#fff" stroke-width="4" stroke-linecap="round"></path></svg>'
  };
  const decorate = () => {
...
/* v431 — favorites + real anonymous article feedback backed by Cloudflare D1. */
(()=>{
  ...
  const clientId=()=>window.TLAnalytics?.clientId?.()||'';
  ...
  const submitVote=async(block,vote)=>{
    const id=clientId();if(!id)return;
    try{
      const r=await fetch('/api/rating',{method:'POST',headers:{'content-type':'application/json','accept':'application/json'},credentials:'same-origin',keepalive:true,body:JSON.stringify({path:votePath(),vote,clientId:id})});
  ...
  document.addEventListener('click',e=>{
    ...
    const next=votes[path]===btn.dataset.railVote?'':btn.dataset.railVote;
...
/* TL_PARTNER_GEO_GATE_V523 */
(function(){
  function run(){
    var nodes=document.querySelectorAll('[data-partner-gated]');
    if(!nodes.length)return;
    fetch('/api/partner-availability',{credentials:'same-origin',cache:'no-store',headers:{'Accept':'application/json'}})
